package com.tarwinderjosan.sampleboundary;
...
public class Boundary {
     ...
     */
    private Rect[] getCanvasRects(int divisions, Rect rect) {
        ...
        for(int i = 0; i < divisions; i++) {
            if(mIsRound) {
                ...
                subSections[i] = new Rect(0, (int) (i * rectHeight), (int) deviceWidth - 1, (int) rectHeight * (i + 1) - 1);
            }
        }
        return subSections;
    }
     ...
     * @param rectNumber The number of the rect to draw in, optional for round wearables         '
     * @param style The type of styling to apply (centered, left justified, etc.
     */
    public void drawRectText(String text, Canvas canvas, Rect r, Paint textPaint, int rectNumber, Paint.Align style) {// Difference in rect height and text height
        ...
        if(style == Paint.Align.LEFT) {
            ...
            if(!mIsRound) {
                canvas.drawText(text, r.left, r.centerY() - yOffset, textPaint);
                    ...
                    canvas.drawText(text, r.left, r.centerY() - yOffset + 5,
                            textPaint);
                }
            }
